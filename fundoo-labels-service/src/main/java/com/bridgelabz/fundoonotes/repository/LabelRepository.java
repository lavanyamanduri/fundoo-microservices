...
public interface LabelRepository extends JpaRepository<Label, Long> {
	...
	@Query(value = "select * from label where label_name=?1",nativeQuery = true)
	...
	@Modifying
	@Query(value = "insert into label(label_name) values(?)",nativeQuery = true)
	void insertData(String label_name,Long user_id);
	
	@Modifying
	@Query(value = "update label set label_name=? where label_id=?",nativeQuery = true)
	void updateData(String labelName,Long label_id,Long user_id);
	
	@Modifying
	@Query(value="delete from label where label_id=?",nativeQuery = true)
